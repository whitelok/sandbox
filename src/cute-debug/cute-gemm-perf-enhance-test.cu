...
karl_gemm_multi_stage(void *Dptr, const void *Aptr, const void *Bptr, int m, int n,
                 int k) {
...
int main(int argc, char *argv[]) {
  ...
  for (int it = 0; it < nt; ++it) {
    ...
    cudaFuncSetAttribute(karl_gemm_multi_stage<decltype(gemm_config)>,
                         cudaFuncAttributeMaxDynamicSharedMemorySize, shm_size);
    karl_gemm_multi_stage<decltype(gemm_config)>
